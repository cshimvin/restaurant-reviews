...
- when the site was deployed to Heroku, the password hash stopped working on the database so I refreshed the passwords in the database.
- when the search page is accessed directly without a search term, a python error is displayed. Added a redirect if the page was accessed directly.
- when a user logs in, featured restaurants weren't shown on the homepage. Added featured restaurants to the homepage template variables.
- an error message was displayed when checking for the value of a session cookie. Added a function to check if the session cookie was present before checking its value.
...
- Go to the repository at https://github.com/cshimvin/restaurant-reviews.git on GitHub
...
Sign up to [MongoDB](https://www.mongodb.com/) and get the database URI as follows:

- create the database name (e.g. restaurant_review)
- create the collection names (e.g. restaurant_types, restaurants, reviews, users)
- add the tables to the collections as detailed in the [database section](#database-structure) above.
- Select the cluster name that has been created
- Click "Connect"
- Click "Connect your application"
- Copy the connection string replacing `<password>` with the password you have created.
...
|     MONGO_URI       |     Your MongoDB URI obtained above    |
|     PORT            |     5000                               |
...
Wrong Way image from https://pxhere.com/en/photo/1021992 - no attribution required.
